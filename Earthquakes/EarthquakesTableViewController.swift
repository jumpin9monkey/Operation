...
class EarthquakesTableViewController: UITableViewController {
    ...
    let operationQueue = OperationQueueCustom()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "earthquakeCell")  as! EarthquakeTableViewCell
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        /*
         Instead of performing the segue directly, we can wrap it in a `BlockOperationDeng`.
         ...
         It's also worth noting that the Observer attached to the `BlockOperationDeng`
         will cause the tableview row to be deselected automatically if the
         `OperationCustom` fails.
        ...
        let operation = BlockOperationDeng {
    ...
    private func getEarthquakes(userInitiated: Bool = true) {
        if let context = fetchedResultsController?.managedObjectContext {
            ...
            */
            let when = DispatchTime.now() + 0.3 * Double(NSEC_PER_SEC)
