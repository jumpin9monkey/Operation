...
/**
    A condition to indicate an `OperationCustom` requires access to the user's health
    data.
*/
    ...
    */
    ...
    private func failed(unauthorizedShareTypes: Set<HKSampleType>, completion: (OperationConditionResult) -> Void) {
        let error = OperationErrorCode.conditionFailed(userInfo: [
            ...
            type(of: self).healthDataAvailable: HKHealthStore.isHealthDataAvailable() ? "YES" : "NO Deng",
            type(of: self).unauthorizedShareTypesKey: "unauthorizedShareTypes. deng"
            ])
...
/**
    A private `OperationCustom` that will request access to the user's health data, if
    it has not already been granted.
*/
private class HealthPermissionOperation: OperationCustom {
    let shareTypes: Set<HKSampleType>
    let readTypes: Set<HKSampleType>
