...
/**
    `URLSessionTaskOperation` is an `OperationCustom` that lifts an `URLSessionTask`
    into an operation.
...
*/
class URLSessionTaskOperation: OperationCustom {
    let task: URLSessionTask
    
    init(task: URLSessionTask) {
        assert(task.state == .suspended, "Tasks must be isSuspended.")
        self.task = task
        super.init()
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        guard let obj = object as? URLSessionTask else{
            return
        }
